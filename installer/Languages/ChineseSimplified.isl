...
SetupFileMissing=安装目录中缺少文件 %1。请修复此问题，或重新获取安装程序。
...
SetupAppRunningError=检测到 %1 正在运行。%n%n请关闭所有实例，然后点击“确定”继续，或点击“取消”退出。
UninstallAppRunningError=检测到 %1 正在运行。%n%n请关闭所有实例，然后点击“确定”继续，或点击“取消”退出。

PrivilegesRequiredOverrideTitle=选择安装模式
PrivilegesRequiredOverrideInstruction=请选择安装模式
PrivilegesRequiredOverrideText1=%1 可以安装给所有用户使用（需要管理员权限），也可以只安装给当前用户使用。
PrivilegesRequiredOverrideText2=%1 可以只安装给当前用户使用，也可以安装给所有用户使用（需要管理员权限）。
...
ErrorTooManyFilesInDir=目录“%1”中的文件过多，无法继续创建文件

ExitSetupTitle=退出安装
ExitSetupMessage=安装尚未完成。如果现在退出，程序将不会被安装。%n%n你可以稍后重新运行安装程序。%n%n确定要退出安装吗？
...
WelcomeLabel2=此向导将在你的电脑上安装 [name/ver]。%n%n建议在继续之前关闭其他应用程序。
...
InfoBeforeClickLabel=准备好后，点击“下一步”继续安装。
WizardInfoAfter=信息
InfoAfterLabel=继续之前，请阅读以下重要信息。
InfoAfterClickLabel=准备好后，点击“下一步”继续安装。
...
InvalidPath=请输入带驱动器盘符的完整路径，例如：%n%nC:\APP%n%n或 UNC 路径，例如：%n%n\\server\share
InvalidDrive=选择的驱动器或 UNC 共享不存在，或无法访问。请选择其他位置。
DiskSpaceWarningTitle=磁盘空间不足
DiskSpaceWarning=安装至少需要 %1 KB 可用空间，但所选驱动器只有 %2 KB 可用。%n%n是否仍要继续？
...
SelectComponentsLabel2=请选择要安装的组件，取消不需要的组件。准备好后点击“下一步”。
FullInstallation=完整安装
CompactInstallation=精简安装
CustomInstallation=自定义安装
NoUninstallWarningTitle=组件已存在
NoUninstallWarning=检测到以下组件已经安装：%n%n%1%n%n取消选择这些组件不会卸载它们。%n%n是否继续？
ComponentSize1=%1 KB
ComponentSize2=%1 MB
...
SelectStartMenuFolderDesc=安装程序应将快捷方式放到哪里？
SelectStartMenuFolderLabel3=安装程序会在以下开始菜单文件夹中创建快捷方式。
...
SelectDiskLabel2=请插入磁盘 %1，然后点击“确定”。%n%n如果此磁盘上的文件位于下方显示位置以外的文件夹，请输入正确路径或点击“浏览”。
PathLabel=路径(&P)：
...
PreviousInstallNotCompleted=上一次安装或卸载尚未完成。你需要重启电脑后继续。%n%n重启后，请重新运行安装程序以完成 [name] 的安装。
CannotContinue=安装程序无法继续。请点击“取消”退出。
ApplicationsFound=以下应用程序正在使用需要更新的文件。建议允许安装程序自动关闭这些应用程序。
ApplicationsFound2=以下应用程序正在使用需要更新的文件。建议允许安装程序自动关闭这些应用程序。安装完成后，安装程序会尝试重新启动它们。
CloseApplications=自动关闭应用程序(&A)
DontCloseApplications=不要关闭应用程序(&D)
ErrorCloseApplications=安装程序无法自动关闭所有应用程序。建议先手动关闭正在使用相关文件的应用程序，然后继续安装。
PrepareToInstallNeedsRestart=安装程序必须重启电脑。重启后，请重新运行安装程序以完成 [name] 的安装。%n%n是否现在重启？
...
ErrorRegOpenKey=无法打开注册表项：%n%1\%2
ErrorRegCreateKey=无法创建注册表项：%n%1\%2
ErrorRegWriteKey=无法写入注册表项：%n%1\%2
ErrorIniEntry=无法在文件“%1”中创建 INI 项。
...
UninstallUnsupportedVer=卸载日志文件“%1”的格式不被当前卸载程序识别，无法卸载
...
UninstalledMost=%1 卸载完成。%n%n部分项目无法移除，可手动删除。
...
ConfirmDeleteSharedFile2=系统显示以下共享文件已不再被任何程序使用。是否让卸载程序移除此共享文件？%n%n如果仍有程序正在使用此文件，移除后可能导致这些程序无法正常工作。如果不确定，请选择“否”。

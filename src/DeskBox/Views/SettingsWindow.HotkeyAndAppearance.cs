...
public sealed partial class SettingsWindow
{
    ...
    // ═══════════════════════════════════════════════════════════════
    //  Search Hotkey (moved from SearchSettingsSection)
    // ═══════════════════════════════════════════════════════════════

    private bool _isRecordingSearchHotkey;

    internal void RefreshSearchHotkeyControls()
    {
        ...
        SearchHotkeyToggle2.IsOn = settings.SearchHotkeyEnabled;

        if (!_isRecordingSearchHotkey)
        {
            SearchHotkeyCaptureButton2.Content = GlobalHotkeyService.FormatGesture(gesture, _localizationService);
        }

        SearchHotkeyStatusText2.Text = settings.SearchHotkeyEnabled
            ? _localizationService.T("Settings.Search.Hotkey.Status.Active")
            : _localizationService.T("Settings.Search.Hotkey.Status.Disabled");
    }

    private void SearchHotkeyToggle2_Toggled(object sender, RoutedEventArgs e)
    {
        // Let SetEnabled own the setting + save + RefreshRegistration.
        App.Current.SearchHotkeyService?.SetEnabled(SearchHotkeyToggle2.IsOn);
        RefreshSearchHotkeyControls();
    }

    private void SearchHotkeyCaptureButton2_Click(object sender, RoutedEventArgs e)
    {
        _isRecordingSearchHotkey = true;
        SearchHotkeyCaptureButton2.Content = _localizationService.T("Settings.Search.Hotkey.Recording");
        SearchHotkeyCaptureButton2.Focus(FocusState.Programmatic);
    }

    private void SearchHotkeyCaptureButton2_KeyDown(object sender, KeyRoutedEventArgs e)
    {
        if (!_isRecordingSearchHotkey)
            return;
        ...
        if (App.Current.SearchHotkeyService is { } svc && !svc.TryApplyGesture(gesture))
        {
            SearchHotkeyStatusText2.Text = _localizationService.T("Settings.Search.Hotkey.Status.Failed");
            return;
        }
    ...
    private void SearchHotkeyCaptureButton2_LostFocus(object sender, RoutedEventArgs e)
    {
        if (_isRecordingSearchHotkey)
            EndSearchHotkeyRecording();
    }

    private void ResetSearchHotkeyButton2_Click(object sender, RoutedEventArgs e)
    {
        var settings = App.Current.SettingsService.Settings;

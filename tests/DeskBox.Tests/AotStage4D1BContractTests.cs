...
public sealed class AotStage4D1BContractTests
{
    ...
    public void LocalizedHeaderAndDescription_UseTheFrozenTypedControlSet()
    {
        ...
        Assert.Equal(123, usages["toolkit:SettingsCard|DescriptionKey"]);
        Assert.Equal(19, usages["toolkit:SettingsExpander|HeaderKey"]);
        Assert.Equal(6, usages["toolkit:SettingsExpander|DescriptionKey"]);
        Assert.Equal(2, usages["TextBox|HeaderKey"]);
        Assert.Equal(303, usages.Values.Sum());
    }
